...
struct DelayLineI {
  ...
  ALuint calcLineLength(const ALfloat length, const uintptr_t offset,
                        const ALfloat frequency, const ALuint extra) {
  ...
  void write(size_t offset, const size_t c, const ALfloat* RESTRICT in,
...
struct VecAllpass {
  ...
  void processFaded(const al::span<ReverbUpdateLine, NUM_LINES> samples,
                    size_t offset, const ALfloat xCoeff, const ALfloat yCoeff,
                    ALfloat fadeCount, const ALfloat fadeStep,
                    const size_t todo);
...
struct T60Filter {
  ...
  void calcCoeffs(const ALfloat length, const ALfloat lfDecayTime,
...
struct EarlyReflections {
  ...
  void updateLines(const ALfloat density, const ALfloat diffusion,
...
struct LateReverb {
  ...
  void updateLines(const ALfloat density, const ALfloat diffusion,
                   const ALfloat lfDecayTime, const ALfloat mfDecayTime,
                   const ALfloat hfDecayTime, const ALfloat lf0norm,
...
struct ReverbState final : public EffectState {
  ...
  struct {
    ...
    ALfloat LFDecayTime{AL_EAXREVERB_DEFAULT_DECAY_LFRATIO *
                        AL_EAXREVERB_DEFAULT_DECAY_TIME};
  ...
  void updateDelayLine(const ALfloat earlyDelay, const ALfloat lateDelay,
                       const ALfloat density, const ALfloat decayTime,
                       const ALfloat frequency);
  void update3DPanning(const ALfloat* ReflectionsPan,
                       const ALfloat* LateReverbPan, const ALfloat earlyGain,
                       const ALfloat lateGain, const EffectTarget& target);
...
bool ReverbState::allocLines(const ALfloat frequency) {
   ...
   */
  ALfloat length{AL_EAXREVERB_MAX_REFLECTIONS_DELAY +
  ...
  totalSamples +=
      mLate.Delay.calcLineLength(length, totalSamples, frequency, 0);
...
inline ALvoid CalcMatrixCoeffs(const ALfloat diffusion, ALfloat* x,
                               ALfloat* y) {
 ...
 * filters.
 */
ALfloat CalcLimitedHfRatio(const ALfloat hfRatio,
   ...
   */
  ALfloat limitRatio{1.0f / (CalcDecayLength(airAbsorptionGainHF, decayTime) *
...
void EarlyReflections::updateLines(const ALfloat density,
...
void LateReverb::updateLines(const ALfloat density, const ALfloat diffusion,
  ...
  const ALfloat late_allpass_avg{std::accumulate(LATE_ALLPASS_LENGTHS.begin(),
   ...
   */
  const ALfloat multiplier{CalcDelayLengthMult(density)};
  ALfloat       length{std::accumulate(LATE_LINE_LENGTHS.begin(),
  ...
  for (size_t i{0u}; i < NUM_LINES; i++) {
     ...
     */
    length +=
        lerp(LATE_ALLPASS_LENGTHS[i], late_allpass_avg, diffusion) * multiplier;
...
void ReverbState::updateDelayLine(const ALfloat earlyDelay,
...
void ReverbState::update3DPanning(const ALfloat*      ReflectionsPan,
  ...
  for (size_t i{0u}; i < NUM_LINES; i++) {
    const ALfloat coeffs[MAX_AMBI_CHANNELS]{earlymat[0][i], earlymat[1][i],
                                            earlymat[2][i], earlymat[3][i]};
    ComputePanGains(target.Main, coeffs, earlyGain, mEarly.PanGain[i]);
  }
  for (size_t i{0u}; i < NUM_LINES; i++) {
    const ALfloat coeffs[MAX_AMBI_CHANNELS]{latemat[0][i], latemat[1][i],
...
void ReverbState::update(const ALCcontext* Context, const ALeffectslot* Slot,
  ...
  mEarly.updateLines(props->Reverb.Density, props->Reverb.Diffusion,
  ...
  mDoFading |= (mParams.Density != props->Reverb.Density ||
                /* Diffusion and decay times influences the decay rate (gain) of
                 * the late reverb T60 filter.
                 */
                ...
                /* HF/LF References control the weighting used to calculate the
                 * density gain.
                 */
...
void VectorScatterRevDelayIn(
    const DelayLineI delay, size_t offset, const ALfloat xCoeff,
    const ALfloat yCoeff, const al::span<const ReverbUpdateLine, NUM_LINES> in,
    const size_t count) {
  ASSUME(count > 0);
...
void VecAllpass::processFaded(
    const al::span<ReverbUpdateLine, NUM_LINES> samples, size_t offset,
    const ALfloat xCoeff, const ALfloat yCoeff, ALfloat fadeCount,
    const ALfloat fadeStep, const size_t todo) {
  const DelayLineI delay{Delay};
  const ALfloat    feedCoeff{Coeff};
...
void ReverbState::earlyFaded(const size_t offset, const size_t todo,
                             const ALfloat fade, const ALfloat fadeStep) {
  const DelayLineI early_delay{mEarly.Delay};
  const DelayLineI main_delay{mDelay};
  const ALfloat    mixX{mMixX};
  const ALfloat    mixY{mMixY};
...
void ReverbState::lateUnfaded(const size_t offset, const size_t todo) {
  ...
  for (size_t j{0u}; j < NUM_LINES; j++) {
    ...
    for (size_t i{0u}; i < todo;) {
      ...
      size_t td{minz(todo - i, minz(main_delay.Mask + 1 - late_delay_tap,
  ...
  for (size_t j{0u}; j < NUM_LINES; j++) {
    ...
    for (size_t i{0u}; i < todo;) {
      ...
      do {
        ...
        mTempSamples[j][i++] = main_delay.Line[late_delay_tap0++][j] * fade0 +
...
void ReverbState::process(const size_t                          samplesToDo,
  ...
  for (size_t c{0u}; c < NUM_LINES; c++) {
    std::fill(tmpspan.begin(), tmpspan.end(), 0.0f);
    MixRowSamples(tmpspan, {B2A[c], numInput}, samplesIn[0].data(),
                  samplesIn[0].size());
...
void EAXReverb_setParami(EffectProps* props, ALCcontext* context, ALenum param,
                         ALint val) {
  switch (param) {
...
void EAXReverb_setParamiv(EffectProps* props, ALCcontext* context, ALenum param,
                          const ALint* vals) {
  EAXReverb_setParami(props, context, param, vals[0]);
}
void EAXReverb_setParamf(EffectProps* props, ALCcontext* context, ALenum param,
                         ALfloat val) {
  switch (param) {
...
void EAXReverb_setParamfv(EffectProps* props, ALCcontext* context, ALenum param,
                          const ALfloat* vals) {
  switch (param) {
...
void EAXReverb_getParami(const EffectProps* props, ALCcontext* context,
                         ALenum param, ALint* val) {
  switch (param) {
  case AL_EAXREVERB_DECAY_HFLIMIT:
...
void EAXReverb_getParamiv(const EffectProps* props, ALCcontext* context,
                          ALenum param, ALint* vals) {
  EAXReverb_getParami(props, context, param, vals);
}
void EAXReverb_getParamf(const EffectProps* props, ALCcontext* context,
                         ALenum param, ALfloat* val) {
  switch (param) {
  case AL_EAXREVERB_DENSITY:
...
void EAXReverb_getParamfv(const EffectProps* props, ALCcontext* context,
                          ALenum param, ALfloat* vals) {
  switch (param) {
  case AL_EAXREVERB_REFLECTIONS_PAN:
...
void StdReverb_setParami(EffectProps* props, ALCcontext* context, ALenum param,
                         ALint val) {
  switch (param) {
  ...
  default:
    context->setError(AL_INVALID_ENUM, "Invalid reverb integer property 0x%04x",
                      param);
  }
}
void StdReverb_setParamiv(EffectProps* props, ALCcontext* context, ALenum param,
                          const ALint* vals) {
  StdReverb_setParami(props, context, param, vals[0]);
}
void StdReverb_setParamf(EffectProps* props, ALCcontext* context, ALenum param,
                         ALfloat val) {
  switch (param) {
  ...
  default:
    context->setError(AL_INVALID_ENUM, "Invalid reverb float property 0x%04x",
                      param);
  }
}
void StdReverb_setParamfv(EffectProps* props, ALCcontext* context, ALenum param,
                          const ALfloat* vals) {
  StdReverb_setParamf(props, context, param, vals[0]);
}

void StdReverb_getParami(const EffectProps* props, ALCcontext* context,
                         ALenum param, ALint* val) {
  switch (param) {
  case AL_REVERB_DECAY_HFLIMIT:
    *val = props->Reverb.DecayHFLimit;
...
void StdReverb_getParamiv(const EffectProps* props, ALCcontext* context,
                          ALenum param, ALint* vals) {
  StdReverb_getParami(props, context, param, vals);
}
void StdReverb_getParamf(const EffectProps* props, ALCcontext* context,
                         ALenum param, ALfloat* val) {
  switch (param) {
  case AL_REVERB_DENSITY:
    *val = props->Reverb.Density;
...
void StdReverb_getParamfv(const EffectProps* props, ALCcontext* context,
                          ALenum param, ALfloat* vals) {
