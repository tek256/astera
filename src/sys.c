...
time_s t_get_time_until(time_s t) { return t - t_get_time(); }
...
s32 t_is_time_before(time_s a, time_s b) { return a < b; }

s32 t_is_time_after(time_s a, time_s b) { return a > b; }
...
void t_end_timert(timer_s *t) { t_end_timer(t->name); }

void t_mark_timert(timer_s *t) { t->timestamp = t_get_time(); }

void t_mark_ttimert(timer_s *t, time_s timestamp) { t->timestamp = timestamp; }

time_s t_get_timert_since(timer_s *t) { return t_get_time() - t->timestamp; }

time_s t_get_timert_until(timer_s *t) { return t->timestamp - t_get_time(); }
